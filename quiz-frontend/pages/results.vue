<template>
  ...
  export default {
    ...
    methods: {
      async loadResult() {
        try {
          const response = await axios.get(`http://localhost:3000/api/quizzes/results/${this.userId}/${this.quizId}`, {
            headers: {
  ...
  try {
    await axios.post('http://localhost:3000/api/feedback/submit', {
      quizId: this.quizId,
      feedbackText: this.feedbackText,  // Verwende hier 'feedbackText'
    }, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('token')}`,
      },
